...
async def _count_user_jobs(db: AsyncSession, user: User) -> int:
    result = await db.execute(select(func.count(Job.id)).where(Job.user_id == user.id))
...
async def _collect_gap_counts(db: AsyncSession, user: User) -> dict[str, int]:
    ...
    result = await db.execute(
        select(Job, CV).join(CV, Job.cv_id == CV.id).where(Job.user_id == user.id)
    )
    rows = result.all()
...
async def _generate_ai_gap_insights(
    ...
    all_items = sorted(gap_counts.items(), key=lambda item: item[1], reverse=True)
    all_gaps: list[str] = []
...
""".strip()
    ...
    for theme in themes:
        ...
        if not isinstance(count, int):
            count = 0
        normalised_themes.append({"label": label, "description": description, "count": count})
